...
public class Level1 extends JFrame {
    ...
    private int finishRow = 3;
    private int finishCol = 3;
    ...
    public void transitionToNextLevel(Level2 level2) {
    ...
    private void movePlayer(int keyCode) {
        ...
        if (newPlayerRow >= 0 && newPlayerRow < ROWS && newPlayerCol >= 0 && newPlayerCol < COLS) {
            if (maze[newPlayerRow][newPlayerCol] == 0 && !playerHistory.contains(new Point(newPlayerCol, newPlayerRow))) {
    ...
    private void checkWin() {
        if (playerRow == finishRow && playerCol == finishCol) {
            mazePanel.repaint();
            int levelPoints = 100; // sau orice altă valoare pe care o dorești pentru nivel
            decoratedLevel.addPoints(levelPoints);
            JOptionPane.showMessageDialog(this, "Ai câștigat! Treci la nivelul următor.");
            Level2 level2 = new Level2(decoratedLevel);
 ...
 /*   public void playMusic() {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> new Level1(pointsDecorator));
    }
